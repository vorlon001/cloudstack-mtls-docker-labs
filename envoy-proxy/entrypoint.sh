...
if head -1 "$DST/server.key" 2>/dev/null | grep -q "RSA PRIVATE KEY"; then
  ...
  echo "[entrypoint] ✓ Конвертация server.key завершена"
else
  echo "[entrypoint] ✓ server.key уже в формате PKCS#8 (BEGIN PRIVATE KEY)"
fi

echo "[entrypoint] Проверяем формат client.key..."
if head -1 "$DST/client.key" 2>/dev/null | grep -q "RSA PRIVATE KEY"; then
  echo "[entrypoint] Обнаружен PKCS#1 (BEGIN RSA PRIVATE KEY). Конвертируем в PKCS#8..."
  openssl pkcs8 -topk8 -nocrypt -in "$DST/client.key" -out "$DST/client8.key"
  mv "$DST/client8.key" "$DST/client.key"
  echo "[entrypoint] ✓ Конвертация client.key завершена"
else
  echo "[entrypoint] ✓ client.key уже в формате PKCS#8 (BEGIN PRIVATE KEY)"
fi
